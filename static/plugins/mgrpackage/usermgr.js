IWF.plugins['usermgr'] = function () {

    var me = this;
    var nav = {icon: 'icon-calendar', title: '用户管理', a: 'usermgr', b: 'usermgr', index: 3, canClose: true};
    var config = {
    ...
    function activeUser(type) {
        ...
        else  var tpl = '<h4>你确定要禁用[' + arr.length + ']条记录吗?</h4>';
        $('body').Dialog({
            title: '提示', tpl: tpl, load: function (e) {
            }, click: function (e) {
                if (e.data.ok) {
                ...
                if (e.data.ok) {
                    ...
                    me.flowGrid.validate({
                        load: function (ve) {
                            ...
                            $.post(me.rootPath + 'user/pwd.data', {
                                id: This.id,
    ...
    var toolConfig = {
        data: [
            {
                title: '新增用户', text: '新增', cls: 'btn btn-blue', key: 'mycost_a', click: function (e) {
                addUser(e);
            }
            },
            {
                title: '复制并新增用户', text: '复制并新增', key: 'mycost_copy', click: function (e) {
                CopyUser(e, 1);
            }
            },
            {
                title: '删除用户', text: '删除', cls: 'btn btn-blue', key: 'mycost_b', menu: [
                {
                    title: '删除用户', text: '逻辑删除', key: 'mycost_b_a', click: function (e) {
                    DelUser(0);
                }
                },
                {
                    title: '删除用户', text: '物理删除', key: 'mycost_b_b', click: function (e) {
                    DelUser(1);
                }
                },
            ]
            ...
            {
                title: '其它', text: '操作', cls: 'btn btn-blue', key: 'mycost_b', menu: [
                {
            ...
            {
                title: '修改用户', text: '修改', cls: 'btn btn-blue', key: 'mycost_c', click: function (e) {
                ModifyUser(e, 0);
            }
            },
            {
                title: '查看用户', text: '详细', key: 'mycost_d', click: function (e) {
                DetailUser(e);
            }
            }
        ]
    ...
    function GetDataSource() {
        ...
        for (var i = 0; i < me.options.dict.length; i++) {
            ...
            if (me.options.dict[i].enname == 'user_status') {
                statusPid = me.options.dict[i].id;
                statusnum = 1;
                //break;
            }
            if (pidnum == 1 && statusnum == 1) break;
        }
        me.ds['sex'] = iwfTool.getDict(me.options.dict, 'pid', pid);
        me.ds['user_status'] = iwfTool.getDict(me.options.dict, 'pid', statusPid);
